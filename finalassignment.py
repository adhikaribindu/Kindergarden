...
class Teacher(Employee):
    ...
    def key_in_assessment_marks(self, student_id, marks):
        pass

    def key_in_term_report(self, student_id, term_report):
        pass
...
class Admin:
    ...
    def add_employee(self):
        ...
        Admin.cur.execute("INSERT INTO employees (name, phone_no, email, position) VALUES (?, ?, ?, ?)", (name, phone_no, email, position))
        Admin.connection_.commit()
        print(f"Employee {name} added successfully.")

    # remove employee
    def remove_employee(self):
        id_to_remove = input("Enter the id of employee to remove: ")
        result = Admin.cur.execute('SELECT id FROM employees WHERE id=?', (id_to_remove,)).fetchone()
        if result:
            Admin.cur.execute('DELETE FROM employees WHERE id = ?', (id_to_remove,))
            Admin.connection_.commit()
    ...
    def update_employee_profile(self):
        ...
        result = Admin.cur.execute('SELECT id FROM employees WHERE id=?', (id_to_update,)).fetchone()
        if result:
            new_position = input("Enter the new position: ")
            Admin.cur.execute('UPDATE employees SET position = ? WHERE id = ?', (new_position, id_to_update))
            Admin.connection_.commit()
    ...
    def view_employee_details(self):
        Admin.cur.execute('SELECT * FROM employees')
        results = Admin.cur.fetchall()
    ...
    def upload_students_list(self):
        ...
        for _ in range(num_students):
            ...
            Admin.students.append(student_data)  # add student to the list
            Admin.cur.execute("INSERT INTO students (name, age, address, contact, section) VALUES (?, ?, ?, ?, ?)", student_data)

        Admin.connection_.commit()
        print("Students list uploaded successfully.")
        print(f"Current students list: {Admin.students}")
        
    # remove student
    def remove_students_names(self):
        names = input("Enter student names to remove : ")

        Admin.cur.execute("DELETE FROM students WHERE name=?",(names,)).fetchone()
        Admin.connection_.commit()
    ...
    def update_students_profiles(self):
        ...
        result = Admin.cur.execute('SELECT id FROM students WHERE id=?', (id_to_update,)).fetchone()
        if result:
            new_age = int(input("Enter new age: "))
            Admin.cur.execute("UPDATE students SET age=? WHERE id=?", (new_age, id_to_update))
            Admin.connection_.commit()
    ...
    def mark_student_attendance(self):
        ...
        Admin.cur.execute("INSERT INTO attendance (student_id, date) VALUES (?, ?)", (student_id, date))
        Admin.connection_.commit()
    ...
    def key_in_assessment_marks_and_term_report(self):
        ...
        Admin.cur.execute("INSERT INTO assessments (student_id, marks, term_report) VALUES (?, ?, ?)", (student_id, marks, term_report))
        Admin.connection_.commit()
